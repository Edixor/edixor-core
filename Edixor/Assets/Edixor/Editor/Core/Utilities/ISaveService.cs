    ...
    where TAsset : ScriptableObject, ISaveCurrentAsset<TData>
{
    ...
    public TData GetCurrentItem()
    {
        ...
        if (settings.Items != null && settings.CurrentIndex >= 0 && settings.CurrentIndex < settings.Items.Count)
        {
            var item = settings.Items[settings.CurrentIndex];
            if (item == null)
            {
                ...
                if (resolvedType != null)
                {
                    ...
                    settings.Items[settings.CurrentIndex] = item;
                }
            }
            return item;
        }
